        ...
        if resulttype == 'file':
            ...
            for res in results:
                if not res['sha256'] in seensha256:
                    # ignore entries without TLSH hashes for now, as they
                    # wouldn't matter for matching and the checksum has
                    # already been recorded in 'fileinfo'
...
def processarchive(scanqueue, resultqueue, sourcesdirectory,
                   unpackprefix, cacheresult, cachedir, processsleep):
    seensha256 = set()
    while True:
        # grab a new task
        task = scanqueue.get()
        ...
        jsonsuccess = False
        # first check if there are already results
        # available and use those instead
        if cacheresult:
        ...
        if cacheresult and jsonsuccess:
                ...
                for f in jsonresults:
                    ...
                    if 'tlshhash' in f and filehash not in seensha256:
        ...
        # cut from the data that is being stored.
        # Add 1 for the '/' of the path
